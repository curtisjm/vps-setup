...
16. **Enables unattended security upgrades.** Writes `/etc/apt/apt.conf.d/20auto-upgrades` — that plus the default `50unattended-upgrades` shipped by Ubuntu is enough to get security-only auto-patching.
17. **Applies kernel hardening.** Writes `/etc/sysctl.d/99-hardening.conf` with the sysctl knobs most often wrong on a stock image: disables IP forwarding, ICMP redirects (accept and send), source routing, enables reverse path filtering, SYN cookies, martian logging, ignores broadcast echoes, TIME_WAIT assassination protection. `sysctl --system` reloads.
18. **Swap file.** If `swapon --show` is empty, creates a 4 GB `/swapfile` (chmod 600, `mkswap`, `swapon`, add to `/etc/fstab`) and sets `vm.swappiness=10` so it's only used under real memory pressure rather than at the whim of the default `60`.
19. **Sets timezone** to `America/Los_Angeles` via `timedatectl`. Pure quality-of-life: log timestamps match wall time.
20. **Prints a summary** listing what was done and the next commands to run.
