...
10. **Configures UFW.** Sets `default deny incoming` / `default allow outgoing`, opens the chosen SSH port with a comment, then `ufw --force enable`. Deliberately additive — no `ufw --force reset` — so rules added later by `07-install-tailscale.sh` (which can narrow SSH to the Tailscale CGNAT range) survive a rerun of 00.
...
- **Additive UFW, not reset.** Other scripts (Tailscale lockdown, possibly your own added rules) layer on top of 00's base rules. A `ufw reset` in 00 would silently undo their work every time you rerun hardening.
