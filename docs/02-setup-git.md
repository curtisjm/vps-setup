...
7. **`~/.ssh/config`: `Host github.com` block.** Adds `IdentityFile`, `IdentitiesOnly yes` etc. if no github.com block exists. If a block already exists, uses `ssh -G github.com` to check whether it resolves to our dedicated key; if not, warns loudly and refuses to edit — you probably configured something on purpose and we don't want to silently point github.com at a different key.
