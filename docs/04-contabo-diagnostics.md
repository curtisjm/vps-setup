...
- **Download speed test fallback.** `speedtest-cli` sometimes can't pick a server (their infra hiccups); the Cloudflare fallback always works but measures only Cloudflare, not arbitrary egress.
