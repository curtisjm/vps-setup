...
10. **`gt doctor`.** Exercises the full stack: Dolt reachable, schema present, identity resolves, workspace detected. Non-zero exit is now a hard fail: if `gt doctor` can't validate the restore, the migration should not claim success.
...
12. **Summary.** Printed only after the checkout sync, tarball verification, daemon start, and `gt doctor` all succeed. Includes bead count, then a checklist:
...
- **Fail fast on code/data mismatch.** Restoring `.dolt-data` onto an old or conflicted `~/gt` checkout is how you get a daemon that "kind of starts" and then fails in weird ways later. Treating `git pull` and `gt doctor` as hard gates prevents that half-broken state from being labeled a successful migration.
- **`gt doctor` as validation.** It's not just a connectivity check — it actively exercises the parts of gastown that would silently break after a bad migration (identity resolution, schema queries, daemon liveness). Non-zero is a stop-the-line failure here, not just a suggestion.
